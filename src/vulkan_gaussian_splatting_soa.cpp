...
VulkanGaussianSplattingSoA::VulkanGaussianSplattingSoA(
    ...
    std::string path)
{
    ...
    if (!imageViewSrc) throw std::runtime_error("VulkanGaussianSplattingSoA: input 0 is not ImageViewSrc");
    ...
    const uint32_t maxMod     = 2;   // binnedGaussians capacity = N * maxMod
    const uint32_t maxBinned  = N * maxMod;
    // Scatter = 512 threads/WG → ceil(N/512) histogram workgroups.
    // Sort capped at 320 but never more than ceil(maxBinned/128).
    ...
    buf3DPos->setName("SoA_Pos3D");
    buf3DRot->setName("SoA_Rot3D");
    buf3DScale->setName("SoA_Scale3D");
    buf3DColAlpha->setName("SoA_ColAlpha3D");
    buf3DShR->setName("SoA_ShR");
    buf3DShG->setName("SoA_ShG");
    buf3DShB->setName("SoA_ShB");
    ...
    // ---- Projected 2D SoA outputs (per-path) ----
    ...
    proj2DPos2D->setName("SoA_ProjPos2D");
    proj2DZ->setName("SoA_ProjZ");
    proj2DBinMask->setName("SoA_ProjBinMask");
    proj2DCovInv->setName("SoA_ProjCovInv");
    proj2DColAlpha->setName("SoA_ProjColAlpha");

    // ---- Stage 1: projection SoA ----
    project3Dto2D = vulkanContext.create<GaussianProjection>(
        "shaders/gsplat/gsplat_projection_soa.comp.spv");
    project3Dto2D->setName("GaussianProjectionSoA");
    ...
    binHistogram->setName("SoA_BinHistogram"); binHistogram->setRecordToZero(true);
    auto binOffsets = std::make_shared<BufferElement<VulkanBuffer<uint32_t>>>(vulkanContext, numBinWGs + 1);
    binOffsets->setName("SoA_BinOffsets");   binOffsets->setRecordToZero(true);
    auto totalCount = std::make_shared<BufferElement<VulkanBuffer<uint32_t>>>(vulkanContext, 1);
    totalCount->setName("SoA_TotalCount"); totalCount->setRecordToZero(true);
    ...
    binCount->setName("GaussianBinningCountSoA");
    ...
    binPrefixSum->setInput(binCount, 0, 3);  // binHistogram (slot 3 of count)
    binPrefixSum->setInput(binCount, 1, 4);  // binOffsets   (slot 4 of count)
    ...
    binPos2D->setName("SoA_BinPos2D");   binPos2D->setRecordToZero(false);
    binZ->setName("SoA_BinZ");           binZ->setRecordToZero(false);
    binBinMask->setName("SoA_BinMask");  binBinMask->setRecordToZero(false);
    binCovInv->setName("SoA_BinCovInv");binCovInv->setRecordToZero(false);
    binColAlpha->setName("SoA_BinCA");   binColAlpha->setRecordToZero(false);
    ...
    binScatter->setName("GaussianBinningScatterSoA");
    ...
    binScatter->setInput(binPrefixSum,  4, 0);   // prefix sums (histogram post-pass2)
    ...
    sortedPos2D->setName("SoA_SortedPos2D");
    sortedZ->setName("SoA_SortedZ");
    sortedBinMask->setName("SoA_SortedBinMask");
    sortedCovInv->setName("SoA_SortedCovInv");
    sortedColAlpha->setName("SoA_SortedColAlpha");

    // ---- Stage 7: gather sorted SoA ----
    // After 8 even radix passes, sorted output is in A buffers.
    gatherSorted = std::make_shared<GeneralComputation<>>(
        vulkanContext, "shaders/gsplat/gsplat_gather_sorted_soa.comp.spv");
    gatherSorted->setName("GatherSortedSoA");
    ...
    gatherSorted->setInput(sortOp,      5, 1);   // sortRadixIdxA (sorted indexes)
    ...
    scratchBounds->setName("SoA_Bounds"); scratchBounds->setRecordToZero(true);

    computeBounds = std::make_shared<GeneralComputation<>>(
        vulkanContext, "shaders/gsplat/gsplat_bin_bounds_soa.comp.spv");
    computeBounds->setName("GaussianComputeBoundsSoA");
...
void VulkanGaussianSplattingSoA::checkInput(ComputeGraphElementPtr input, int index) {
    if (index == 0 && !std::dynamic_pointer_cast<ImageViewSrc>(input))
        throw std::runtime_error("VulkanGaussianSplattingSoA: input 0 must be ImageViewSrc");
    if (index == 1 && !std::dynamic_pointer_cast<CameraUboType>(input))
        throw std::runtime_error("VulkanGaussianSplattingSoA: input 1 must be CameraUboType");
    if (index > 1)
        throw std::runtime_error("VulkanGaussianSplattingSoA: input index out of range");
}

void VulkanGaussianSplattingSoA::_setup(VulkanContext& vulkanContext, uint32_t numberPaths) {
    numberOfPaths = numberPaths;

    // Pre-fill sort value buffers with sentinel 0xFFFFFFFF for the first frame.
    // _record fills them again at the end of every frame for the next frame.
...
void VulkanGaussianSplattingSoA::_record(VkCommandBuffer commandBuffer, uint32_t pathId) {
    // Pre-fill sort value buffers for the NEXT frame (same sentinel strategy as AoS).
    ...
    // Transition rendered image back to the correct present/general layout.
...
void VulkanGaussianSplattingSoA::loadSPZModel(std::string path) {
    ...
    std::cout << "SoA: loaded " << number_of_gaussians << " gaussians from " << path << "\n";
}
