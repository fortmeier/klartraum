...
const std::vector<Vertex3>& getVertices(DrawBasicsType type) {
    switch (type) {
    case DrawBasicsType::Triangle:
        return vertices_triangle;
    case DrawBasicsType::Axes:
        return vertices_axes;
    default:
        throw std::runtime_error("DrawBasicsType not implemented");
    }
}

const VkPrimitiveTopology getTopology(DrawBasicsType type) {
    switch (type) {
    case DrawBasicsType::Triangle:
...
VkSemaphore DrawBasics::draw(uint32_t currentFrame, VkFramebuffer framebuffer, VkSemaphore imageAvailableSemaphore) {
    auto device = backendVulkan.getDevice();
...
void DrawBasics::createGraphicsPipeline() {
    ...
    inputAssembly.topology = getTopology(type);
    ...
    pipelineInfo.renderPass = backendVulkan.getRenderPass();
...
void DrawBasics::createVertexBuffer() {
    ...
    allocInfo.memoryTypeIndex = backendVulkan.findMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void DrawBasics::createCommandPool() {
...
void DrawBasics::createCommandBuffers() {
    auto device = backendVulkan.getDevice();
